...
Finally, the functions $R_8^{(2)}$\! and $R_9^{(2)}$\! are expected to involve contributions proportional to $\zeta_4$. Since the remainder function is smooth in collinear limits, namely
\begin{equation} \label{eq:remainder_smooth_collinear_limits}
R_n \xrightarrow[]{p_i || p_{i+1}} R_{n-1} \, ,
\end{equation} 
these contributions can be determined analytically by computing the difference between the collinear limit of each of these functions and the remainder function involving one fewer leg. Thus, we first fix the $\zeta_4$ contribution to $R_8^{(2)}$\! by comparing its collinear limit to the known value of $R_7^{(2)}$\!~\cite{Golden:2014xqf}, after which we can similarly fix the $\zeta_4$ contribution to $R_9^{(2)}$\!. 
...
Using this procedure, we have analytically determined the $\zeta_4$ contributions to both $R_8^{(2)}$\! and $R_9^{(2)}$\!. The corresponding coefficients of $\zeta_4$ can be found in the ancillary files, but aren't meaningful in isolation since they depend on the polylogarithmic representation we chose to represent the nonclassical and classical contributions to these amplitudes. This completes our determination of these amplitudes.
