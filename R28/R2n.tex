...
In Figure~\ref{fig:symm_to_collinear_lines}, we see that the remainder function monotonically decreases as one approaches successive collinear limits along the lines parametrized by~\eqref{eq:line_parametrization}. (It is worth noting, however, that the remainder function is not at a local maximum at these dihedrally-symmetric points.) The line segments that appear in Figure~\ref{fig:symm_to_collinear_lines} all also take approximately the same shape. This can be contrasted with Figure~\ref{fig:random_point_to_collinear_lines}, where we have plotted the remainder function on the line starting at a randomly chosen point in nine-particle kinematics, given by the momentum twistor matrix
\begin{equation}
\left(
