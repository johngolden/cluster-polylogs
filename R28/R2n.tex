...
Only a handful of results beyond one loop are known in planar $\mathcal{N} = 4$ sYM theory for more than seven particles. Most notably, these include the symbols of the two-loop maximally-helicity-violating (MHV) and next-to-MHV (NMHV) amplitudes, which can be computed at any multiplicity using the methods of~\cite{CaronHuot:2011ky,Zhang:2019vnm,He:2020lcu,He:2020vob}. In addition, a number of higher-point Feynman integrals that contribute to the amplitudes in this theory are known at two (and higher) loops~\cite{Bourjaily:2018aeq,Henn:2018cdp,He:2020uxy,He:2020lcu,Bourjaily:2021lnz}, while the two-loop MHV amplitude has been explored numerically and in special kinematic configurations at higher multiplicities~\cite{Anastasiou:2009kna,Brandhuber:2009da,DelDuca:2010zp,Heslop:2010kq,Gaiotto:2010fk}.
...
As part of this analysis, we investigate whether the nonclassical parts of the eight- and nine-particle remainder functions can be decomposed into functions associated with the larger subalgebras of $\Gr(4,8)$ and $\Gr(4,9)$. In~\cite{Golden:2018gtk}, it was shown that the nonclassical part of the seven-particle remainder function could be decomposed into functions associated with the $A_4$, $A_5$, and $D_5$ subalgebras of $\Gr(4,7)$, in addition to the $A_2$ and $A_3$ decompositions found in~\cite{Golden:2014xqa}. We find here that the eight-particle remainder function can be decomposed in terms of the same $A_5$ function that the seven-particle remainder function was constructible in terms of, and that the two-loop contribution to $R_{n<10}$ admits a unique decomposition in terms of the function $f_{A_3}^{+-}$ defined in~\cite{Golden:2014xqa}, which has a different signature under the $A_3$ automorphism group than the original $A_3$ function identified in~\cite{Golden:2014xqa}. This leads us to expect that there exists a unique decomposition in terms of $f_{A_3}^{+-}$ at all $n$.

We also make use of our new expressions to explore the behavior of the remainder function in the positive region. We focus on the dihedrally-symmetric point that exists in the positive region at each particle multiplicity, and plot the behavior of the remainder function as one moves from this symmetric point into successive collinear limits, all the way down to five-point kinematics. This allows us to see the remainder function smoothly limiting to the appropriate lower-point values along multiple lines, providing a cross-check on our results. 
