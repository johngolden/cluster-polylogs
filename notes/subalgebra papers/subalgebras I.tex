...
\subsection{Subalgebra Structure and the \pdfeq{A_2} function}
...
As we saw in section~\ref{sec:subalgebras_associahedra}, cluster algebras are endowed with subalgebras which can be generated by mutating on restricted sets of nodes. In particular, $A_2$ will subalgebras arise whenever this is done on a pair of nodes that are connected by an arrow---thus, it is easy to see they will occur ubiquitously as subalgebras of $\Gr(4,n)$. Much less trivially, all of the information contained in $\delta_{2,2}\big(R^{(2)}_n\big)$ turns out to be encoded in a single function that can be defined on these $A_2$ subalgebras.

Let us explore what this means. We define the $A_2$ function---which can be thought of as a `function on an oriented graph'---to be
...
\end{align}
where the $\x_i$ are defined in terms of $x_1$ and $x_2$ as in eq.~(\ref{def:a2-xcoords}), and the skew-dihedral sum indicates subtracting the dihedral flip $\x_i \to \x_{6-i}$ and taking a cyclic sum $i=1$ to $5$. This function differs in a few salient respects from the $A_2$ function defined in~\cite{Golden:2014xqa} (which we will discuss below), but has the same cobracket
...
\end{equation}
Remarkably, it was there shown that the nonclassical component of the $n$-particle remainder function is expressible entirely in terms of this function, evaluated on some collection of the $A_2$ subalgebras of $\Gr(4,n)$; that is,
\begin{equation}
\delta_{2,2}\big( R_n^{(2)} \big) = \sum_{(x_i\to x_j) \subset \Gr(4,n)} c_{ij}\ \delta_{2,2} \big(f_{A_2}(x_1 \to x_2)\big)
\end{equation}
for some rational coefficients $c_{ij}$. Moreover, the terms in this sum can be arranged into $A_3$ subalgebras, giving rise to a natural $A_3$ function of the form
\begin{equation}
f_{A_3}(x_1 \to x_2 \to x_3) \sim \sum_{(x_i\to x_j) \subset A_3} d_{ij}\ f_{A_2}(x_1 \to x_2),
\end{equation}
where the $d_{ij}$ are some rational coefficients that we treat in full detail in the next section. For now, we merely highlight the fact that these coefficients can be not only in such a way that
...
\end{equation}
but also such that $\delta_{2,2}\big(f_{A_3} \big)$ contains only terms $\{\x_i\}_2 \otimes \{\x_j \}_2$ in which $\x_i$ and $\x_j$ appear together in at least one cluster of $\Gr(4,n)$. This surprising property of the amplitude is thus made manifest in the decomposition~\eqref{eq:remainder_A3_deomposition}.

In the following sections we analyze these decompositions systematically, and find they can be extended to much larger subalgebras in seven-particle kinematics. But first, let us return to the definition of cluster polylogarithms, which we are now in a position to make precise. \newpage
