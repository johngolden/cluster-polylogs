...
The infrared structure of planar ${\cal N} = 4$ SYM is captured to all orders by the BDS ansatz~\cite{}. In four- and five-particle kinematics the BDS ansatz also constitutes the full kinematic dependence of the amplitude; for six or more particles, it receives a dual-conformally invariant correction. In this case of the MHV amplitude, this correction is often computed in the form of the $n$-particle remainder function $R_n$, defined by
\begin{equation}
...
\end{equation}
where $g^2 = \frac{g_{\text{YM}}^2 N_c}{16 \pi^2}$. We have used the fact that $R_n^{(1)} = 0$, since the BDS ansatz accounts for the one loop MHV amplitude. 

The remaining $L$-loop contributions to the remainder function are expected to be generalized polylogarithms of weight $2L$. These

The symbol of the two-loop contribution to the remainder function is known to all $n$, due to an impressive computation that leveraged the superconformal symmetry of this theory~\cite{}.
