...
The infrared structure of planar ${\cal N} = 4$ SYM is captured to all orders by the BDS ansatz~\cite{}. In four- and five-particle kinematics the BDS ansatz also completely accounts for the finite part of the amplitude, while for six or more particles it is corrected by a finite dual-conformally invariant function~\cite{}. In the case of the MHV amplitude, this correction is often computed in the form of the $n$-particle remainder function $R_n$, defined by
\begin{equation}
...
\end{equation}
where $g^2 = \frac{g_{\text{YM}}^2 N_c}{16 \pi^2}$. We have here used the fact that $R_n^{(1)} = 0$, since the BDS ansatz accounts for the complete one loop MHV amplitude at all $n$. 

The remaining $L$-loop contributions to the remainder function are expected to be expressible in terms of generalized polylogarithms~\cite{Chen,FBThesis,Gonch} of uniform transcendental weight $2L$. This space is spanned by the functions
...
\end{equation}
where $G(;z) \equiv 1$, and the transcendental weight of each function corresponds to its number of indices $k$. In particular, the remainder function is expected to be a pure function of this type, meaning that all its kinematic dependence appears in the indices $a_i$ and argument $z$, not in the rational prefactors multiplying these functions. This is known to be true at two loops, due to an impressive all-$n$ computation that leveraged the superconformal symmetry of this theory~\cite{CaronHuot:2011ky}, as well as through six loops in six-particle kinematics~\cite{} and through four loops in seven-particle kinematics~\cite{Dixon:2016nkn}. 

In addition to being generalized polylogarithms, loop-level contributions to the remainder function exhibit a great deal of cluster-algebraic structure. For this reason, they were christened `cluster polylogarithms' in~\cite{Golden:2014xqa}, where this was meant to denote the fact that their symbol is naturally expressible in terms of cluster \acoords, while their Lie cobracket is naturally expressible in terms of cluster \xcoords\ (in a way that will be made precise below). We propose that this definition be refined to include only functions that respect the automorphism group and the cluster adjacency principle of~\cite{}. The rest of this section is devoted to making these properties precise, for which purpose we first describe the motivic structure of polylogarithms. 

\subsection{The Symbol and Cobracket} \label{sec:coproduct}

The space of generalized polylogarithms defined by~\eqref{eq:multiple_polylogs} is colossally overcomplete. This is both because $a_i$ and $z$ are allowed to be arbitrarily complicated algebraic functions, and because these functions satisfy a shuffle algebra. Namely, when two polylogarithms share an argument $z$, their product can be re-expressed as the sum of functions 
\begin{equation}
G(a_1,\dots,a_{k_1};z)\ G(a_{{k_1}+1},\dots,a_{k_1+k_2};z)  = \sum_{\sigma \in \Sigma(k_1,k_2)} G(a_{\sigma(1)},\dots,a_{\sigma(k_1+k_2)};z),
\end{equation}
where $\Sigma(k_1,k_2)$ denotes all shuffles between the sets of integers $\{1,\dots,k_1\}$ and $\{k_1+1,\dots,k_1+k_2\}$ (that is, all ways of interleaving these two sets such that the ordering within each set is maintained). This shuffle algebra arises 

This overcompleteness gives rise to a rich space of identities. This can already be seen at the level of classical polylogarithms, which correspond to the cases
\begin{equation}
...
Fortunately, all identities between polylogarithms are (conjecturally) generated by shuffle and stuffle relations~\cite{2011arXiv1102.1312B,2015arXiv151206409B}. These are trivialized by the symbol map, which can be defined iteratively in terms of the total derivatives of these functions. Taking the total derivative of~\eqref{eq:multiple_polylogs}, we have
...
\end{equation} 
where $a_0 \equiv z$ and $a_{k+1} \equiv 0$, and the notation $\hat{a}_i$ indicates this index should be omitted~\cite{GoncharovMixedTate}. The symbol map is then defined to be
\begin{equation} \label{eq:symbol_def}
...
\end{equation} 
The entries of the resulting $k$-fold tensor product are referred to as symbol letters. These symbol letters inherit the properties of (arguments of) logarithms, and can therefore be expanded into a multiplicatively independent basis of symbol letters (the `symbol alphabet'). Complicated polylogarithmic identities are thereby reduced to identities between logarithms, at the cost of losing information about the boundary of integration in~\eqref{eq:multiple_polylogs}.
 
The symbol also captures the analytic structure of polylogarithms, insofar as it encodes their (iterated) discontinuity structure. Namely, for generic indices $a_i$ and argument $z$, the branch cut structure of the general polylogarithm defined in~\eqref{eq:multiple_polylogs} is dictated by the symbol letters appearing in its first entry. These functions have nonzero monodromy when analytically continued around points where these symbol letters vanish or become infinite (sharing the monodromies of logarithms with these arguments), and are analytic everywhere else~\cite{Duhr:2012fh}. This monodromy can itself have branch cuts that the original function did not have, as encoded in the second entry of the symbol, \emph{ad infinitum}. For special values of $a_i$ and $z$, some of this information is lost due to the fact that higher-weight transcendental constants (such as Riemann $\zeta$ values) are in the kernel of the symbol~\cite{}. However, this information is retained by the coaction~\cite{}, and can be recovered by specifying the integration constant at a kinematic point~\cite{}. We defer consideration of these transcendental constants to a follow-up paper~\cite{}.
...
\end{equation}
where $\rho(s_1) \equiv s_1$. This operator projects onto the component of a symbol that cannot be written as a product of lower-weight polylogarithms.

of a weight $k$ function $f$ can be calculated as
\begin{equation}
\delta(f) \equiv \sum_{i=1}^k (\rho_i \wedge \rho_{k-i})\rho(f),
\end{equation}
where the 

The salient property of the cobracket here is that it isolates the component of any polylogarithm that can be written in terms of classical polylogarithms (with algebraic arguments). 
