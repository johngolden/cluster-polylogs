...
While the (nonclassical part of the) two-loop MHV amplitudes in planar ${\cal N} =4$ sYM~\cite{Golden:2014xqa} are already known to be $A_2$- and $A_3$- constructible, it is interesting to ask whether they can be constructed in terms of cluster polylogarithms on larger algebras. In particular, using the construction described above, we can ask this question of the two-loop seven-point MHV amplitude, $R^{(2)}_7$---itself a cluster polylogarithm associated with the Grassmannian $\Gr(4,7)$. The subalgebra and automorphism structure of $\Gr(4,7)$ strongly constrains not only the number of such subalgebra-constructible functions available in this space, but also uniquely dictates the form these decompositions can take, up to an overall rescaling. We show that decompositions exist over the $D_5$, $A_5$, and $A_4$ subalgebras of $\Gr(4,7)$, and take particularly simple forms:
\begin{align}
...
\end{align}
where the trailing dots indicate a contribution of purely classical polylogarithms, and the relevant functions and notation are defined section~\ref{sec:r27-sub-constructibility}. These representations of $R^{(2)}_7$ are therefore far more canonical than those available through the $A_2$ or $A_3$ functions used in~\cite{Golden:2014xqf}, as those satisfy numerous identities. 
...
\end{equation}
where $\smash{f_{A_4}^{+-}}$ is the \emph{same} function that appeared in the $A_4$ decomposition of $\smash{f_{D_5}^{---}}$\!. Therefore we find an $A_4\subset A_5$ representation of $R^{(2)}_7$:
...
\end{equation}
which bears a striking resemblance to the $A_4\subset D_5 \subset \Gr(4,7)$ representation found in eq.~(\ref{eq:r27-d5-a4-decomp1}). In fact, the two sums are identical. Each of the seven $A_5$ subalgebras contains eight $A_4$ subalgebras, therefore this $A_4\subset A_5\subset \Gr(4,7)$ sum results in 56 $A_4$ subalgebras contributing. These are the exact same 56 $A_4$ subalgebras which contributed in eq.~(\ref{eq:r27-d5-a4-decomp1}) (the overall sign difference between the two equations is simply due to the orientations of the $A_5$ and $D_5$ subalgebras inside $\Gr(4,7)$). We now turn to a discussion of these overlapping decompositions of $R^{(2)}_7$. 
