...
While cluster polytopes give us a nice pictorial way to think about the relations between different clusters in a cluster algebra, their interior can also be identified with the `positive region' where all (cyclically ordered) \acoords\ (and consequently \xcoords) are positive~\cite{ArkaniHamed:2012nw,Drummond:2018dfd}. Thus, in the case of Grassmannian cluster algebras, the positive Grassmannian $\Gr_+(k,n)$ (restricted to real values) consists of all points in the interior of the cluster polytope. Each cluster, moreover, can be thought of as providing a coordinate system in which the full positive region is spanned by allowing each \xcoord\ to range from $0$ to $\infty$. For a more in-depth discussion of this way of thinking about cluster polytopes, see~\cite{Drummond:2018dfd}.
