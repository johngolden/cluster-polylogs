...
\begin{multline}
	\delta_{2,2}\left(\sum_{E_6^{+++}} 2 f_{A_3}^{--}(x_1 \to x_2 (1 + x_3)\to \frac{x_3 x_4 x_5}{1 + x_{5,3}})- 
f_{A_3}^{--}(x_1\to x_2 (1 + x_3)\to \frac{x_3 x_4 x_5 x_6}{1 + x_{6,5,3}})\right. \\ \left.\phantom{\sum_{E_6^{+++}}}- 
f_{A_3}^{--}(x_1\to \frac{x_2 (1 + x_{6,5,3,4})}{1 + x_{6,5}}\to \frac{x_3 x_5}{(1 + x_5) (1 + x_{6,5,3})})\right) = 0.
\end{multline}
...
We are now allowed to tune the remaining parameters $c_1$ and $c_2$ in order to endow $f_{D_5}^{---}$ with additional properties. In continuing with the theme of subalgebra-constructibility we can look for decompositions of $f_{D_5}^{---}$ in terms of subalgebras of $D_5$. This follows exactly the same procedure for finding $D_5$-decompositions of $R^{(2)}_7$, instead now we are trying to find $A_4$-, $D_4$-, or $A_3$-decompositions of $f_{D_5}^{---}$. It turns out that there are two different possibilities: one of which involves $A_4$ subalgebras and the other involving $A_3$.

It is worth highlighting that $f_{D_5}^{---}$ does not satisfy cobracket-level cluster adjacency for generic values of $c_1$ and $c_2$; however, it can be given this property by choosing
\begin{equation}
	c_2 = -\frac{6}{5} + \frac{8}{5} c_1.
\end{equation}
This choice of parameters makes $f_{D_5}^{---}$ an $A_3$-constructible function:
\begin{align}
...
\end{align}
The cobracket-level cluster adjacency of $f_{A_3\subset D_5}^{---}$ is thus manifest, as $f_{A_3}^{--}$ is the simplest function exhibiting this property.

The $A_4$-decomposition of $f_{D_5}^{---}$ is obtained by setting
\begin{equation}
...
\end{equation} 
Regrettably $f_{A_4}^{+-}$ does not admit an $A_3$-representation, and so does not satisfy cobracket-level adjacency. Still, from a symmetry perspective this $A_4\subset D_5$ representation of $R^{(2)}_7$ is quite remarkable, and can be described as: 
\begin{itemize}
...
Having provided cluster algebraic motivations for certain propitious choices of $c_1$ and $c_2$, we now examine whether $f_{D_5}^{---}$ can be endowed with interesting behaviour under kinematic limits. In particular, we know that the nonclassical portion of $R^{(2)}_7$ must vanish under the $7\to6$ collinear limit, as $R^{(2)}_6$ is purely classical. We can then constrain $c_1$ and $c_2$ by requiring as many of the 14 $f_{D_5}^{---}$ be well-behaved and have vanishing nonclassical components under the collinear limit. Doing so surprisingly sets
\begin{equation}
	c_1 = 0, \qquad c_2 = -\frac{6}{5}.
\end{equation}
Since this is compatible with the $A_3^{--}$ representation of $f_{D_5}^{---}$, we can say that the collinear limit favors this particular decomposition, with an additional constraint on $c_1$. However even with this choice, the nonclassical component of only 10 of the 14 $f_{D_5}^{---}$ functions vanishes. The remaining 4 functions are all poorly behaved under the collinear limit, and cancel off each other in two sets of pairs. 

\draftnote{other interesting kinematic limits}

Having now exhausted the different $D_5$-based representations of $R^{(2)}_7$ we now turn to $A_5$.
