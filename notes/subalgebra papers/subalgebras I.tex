...
A growing body of evidence suggests that cluster algebras (or their generalizations) have a central role to play in multi-loop scattering amplitudes in the planar limit of ${\cal N} = 4$ supersymmetric Yang-Mills (SYM) theory. All known loop-level amplitudes in this theory are expressible in terms of cluster coordinates~\cite{Golden:2013xva,Golden:2013lha,Golden:2014xqa,Golden:2014pua,Dixon:2013eka,Dixon:2014voa,Dixon:2014iba,Drummond:2014ffa,Dixon:2015iva,Caron-Huot:2016owq,Dixon:2016nkn,Drummond:2017ssj}, as are certain classes of Feynman integrals~\cite{Drummond:2010cz,Drummond:2017ssj,Bourjaily:2018aeq,Henn:2018cdp}, some of which have been computed to all loop orders~\cite{Caron-Huot:2018dsv}. Even more strikingly, the symbols of these integrals and amplitudes (once appropriately normalized) are believed to involve only cluster coordinates that appear together in clusters of Gr(4,$n$) in adjacent entries~\cite{Drummond:2017ssj}, an observation that is also borne out by all amplitudes that have been computed in this theory~\cite{all_orders_adjacency}. Thus, cluster algebras seem to combinatorially realize much of the analytic structure of these amplitudes, encoding their locality in a non-obvious way.  

The fact that cluster algebras appear in this context is not totally surprising, insofar as they are dual to plabic graphs, which describe the integrands of this theory to all orders~\cite{ArkaniHamed:2012nw}. In particular, physical singularities . However, it is not obvious that new branch points integration won't generate ---and indeed, it has recently been seen that Feynman integrals contributing to eight- and higher-particle amplitudes involve algebraic roots when expressed in terms of cluster coordinates, corresponding to internal loop momenta becoming equal~\cite{Prlina:2017azl,Bourjaily:2018aeq,Henn:2018cdp}. This obfuscates the connection between amplitudes and cluster algebras (which only involve rational transformations), as does the eventual appearance of functions beyond polylogarithms in this theory~\cite{}. Both complications point to the need for some generalization of cluster algebras (or of the role cluster coordinates play in iterated integrals) if analogous algebraic structure is to be found at higher loops and particle multiplicities.

There is reason to be optimistic this structure exists, due to the sheer number of ways cluster-algebraic structure appears in the maximally-helicity-violating (MHV) amplitudes of this theory at low loop orders. The two-loop symbol of these amplitudes has been computed to all $n$~\cite{CaronHuot:2011ky}, providing a rich source of data that can be probed for cluster-algebraic properties. In addition to having symbol alphabets composed entirely of cluster coordinates in Gr(4,$n$), it has been shown that the `nonclassical' part of these amplitudes is uniquely determined by a combination of physical and cluster-algebraic properties~\cite{Golden:2014pua}. , a pair of functions can be associated with the $A_2$ and $A_3$ subalgebras of Gr(4,$n$), in terms of which this nonclassical component of these amplitudes can be geometrically decomposed into sums over these subagebras~\cite{Golden:2014xqa}. Their remaining `classical' part can always be written as products of classical polylogarithms with arguments that~\cite{Golden:2014xqf}.\draftnote{should we phrase this as a conjecture, or as it's stated now?} .


The appearance of cluster algebras in loop-level a theory was first noted at two loops~\cite{Golden:2013xva,Golden:2013lha,Golden:2014xqa,Golden:2014pua}, where the symbol of the $n$-point maximally-helicity-violating (MHV) amplitude~\cite{CaronHuot:2011ky} was observed to involve only cluster coordinates on the grassmannian Gr(4,$n$). This observation has since been borne out to higher loops in six- and seven-particle kinematics in both MHV and next-to-MHV (NMHV) amplitudes~\cite{Dixon:2013eka,Dixon:2014voa,Dixon:2014iba,Drummond:2014ffa,Dixon:2015iva,Caron-Huot:2016owq,Dixon:2016nkn,Drummond:2017ssj}, as well as in certain classes of Feynman integrals, some of which have been computed to all loop orders~\cite{Drummond:2017ssj,Bourjaily:2018aeq,Caron-Huot:2018dsv,Henn:2018cdp}. Even more strikingly, it has been conjectured that pairs of these cluster coordinates only appear in the adjacent entries of the symbol of appropriately-normalized amplitudes when they also appear together in a cluster in Gr(4,$n$)~\cite{Drummond:2017ssj}.  
...
Perhaps the appearance of cluster algebras in this context isn't so surprising, given that integrands in this theory are described to all orders by the positive grassmannian $\Gr^+(k,n)$~\cite{}. Each on-shell diagram comes equipped with a cluster algebra associated with the variables that label its faces, which belong to the nodes of its dual graph.  


The remainder of this paper is organized as follows. In section~\ref{sec:brief_intro} we provide a self-contained introduction to cluster algebras and the main ways in which they appear in the planar limit of ${\cal N}=4$ SYM theory. This section is intended to fill a pedagogical gap in the physics literature, and can be skipped by those who are familiar with recent developments at the intersection of these topics. In section~\ref{sec:cluster_polylog_MHV_review} we 
...
Terminology guide (let $A$ be a cluster algebra, $B_i$ be a rank $i$ subalgebra of $A$, and $i_1<i_2<\ldots$):
\begin{itemize}
	\item ``$B_i{\subset}A$-constructible $\a/\x$-polylog': an automorphic $\a$- or $\x$-polylog on $A$ that is composed of automorphic $\a$- or $\x$-polylogs on $B_i$,
	\item ``$B_{i_1} {\subset} B_{i_2}{\subset}\ldots{\subset}A$-constructible $\a/\x$-polylog'': iterating the above definition for multiple nestings of subalgebras of decreasing rank.
\end{itemize}
