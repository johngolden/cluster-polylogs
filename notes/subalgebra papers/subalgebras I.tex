...
and
\begin{align}
\sum_{A_5\subset E_6} f_{A_5}(x_i \to \ldots) &\equiv \frac{1}{4} \sum_{i=0}^6\sum_{j=0}^1\sum_{k=0}^1 \ \mathbb{Z}_{2,E_6}^k \circ \tau_{E_6}^j \circ \sigma_{E_6}^i \Big(f_{A_5}(x_i \to \ldots) \Big) \\ 
...
\begin{align}
\delta_{2,2}\big(R^{(2)}_7 \big) = \sum_{D_5\subset \Gr(4,7)} \delta_{2,2} \Big( f_{D_5}^{---}\left(
\begin{gathered}
    \begin{xy} 0;<1pt,0pt>:<0pt,-1pt>::
      (0,20) *+{x_1} ="1",
      (30,20) *+{x_2} ="2",
      (72,20) *+{\begin{aligned} \frac{ x_3 x_5 x_6}{x_{6,5}} \end{aligned}} ="3",
      (114,0) *+{\color{white} x} ="4",
      (114,40) *+{\color{white} x} ="5",
      (136,0) *+{\begin{aligned} \frac{1}{x_6 (1 + x_5)} \end{aligned}} ="6",
      (121,40) *+{\begin{aligned} \frac{x_{6,5}}{x_5}\end{aligned}} ="7",
      "1", {\ar"2"},
      "2", {\ar"3"},
      "3", {\ar"4"},
      "3", {\ar"5"},
...
\begin{gathered}
    \begin{xy} 0;<1pt,0pt>:<0pt,-1pt>::
      ...
      (30,15) *+{x_2} ="2",
      (60,15) *+{x_3} ="3",
      (90,0) *+{\color{white} x_i} ="4",
      (90,30) *+{\color{white} x_i} ="5",
      (90,2) *+{x_4} ="6",
      (90,28) *+{x_5} ="7",
      "1", {\ar"2"},
      "2", {\ar"3"},
...
\end{equation}
The \xcoords\ in~\eqref{eq:R27_D5_decomposition} are the $E_6$ coordinates defined in~\eqref{eq:E6_x_def}, while the \xcoords\ in~\eqref{eq:fd5-init} should be thought of as $D_5$ coordinates that take different values when evaluated on the $D_5$ subalgebras of $E_6$. The coefficients $c_1$ and $c_2$ represent the two remaining degrees of freedom in our ansatz, which drop out of the sum~\eqref{eq:R27_D5_decomposition}.

Clearly the coefficients $c_1$ and $c_2$ deserve more attention, but before this we turn to $A_5$\dots
...
\subsection{Nesting $A_4$ and $A_3$ Functions in $D_5$ and $A_5$}
