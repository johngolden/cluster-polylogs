...
\begin{align}
	R^{(2)}_7 &= {\color{white} - }\frac{1}{5} \sum_{D_5\subset \Gr(4,7)} \sum_{A_4\subset D_5} f_{A_4}^{+-}(x_1\to x_2 \to x_3 \to x_4) + \dots \\
	& = -  \frac{1}{5}  \sum_{A_5\subset \Gr(4,7)} \sum_{A_4\subset A_5} f_{A_4}^{+-}(x_1\to x_2 \to x_3 \to x_4) + \dots
\end{align}
where the trailing dots indicate that this equality holds up to the contribution of purely classical polylogarithms. In fact, the required classical contribution is the \emph{same} in both lines, as strict equality holds between the two nested sums over $f_{A_4}^{+-}$ (including the minus sign in front of the second). This in itself is rather phenomenal 
