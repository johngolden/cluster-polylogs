...
\end{equation}
where $\Sigma(k_1,k_2)$ denotes the set of all shuffles between the sets of integers $\{1,\dots,k_1\}$ and $\{k_1+1,\dots,k_1+k_2\}$ (that is, all ways of interleaving these two sets such that the ordering of the elements within each one is maintained). The stuffle algebra naturally arises when generalized polylogarithms are re-expressed as infinite sums, and represents the freedom to split up unordered summation indices into nested sums where these indices are ordered; see~\cite{Duhr:2014woa} for more about relations of this kind.
...
As stated in the introduction, the two-loop MHV amplitudes of this theory exhibit different forms of cluster-algebraic structure at the level of their cobracket, their symbol, and as full functions. The first aspect of this structure, observed in~\cite{Golden:2013xva} and further explored in~\cite{Golden:2014xqa,Golden:2014xqf,Golden:2014pua}, concerns the building blocks that appear at each of these levels:
\begin{itemize}
\item[$\bullet$] The cobracket of $R^{(2)}_n$ can be written in terms of elements of the Block group that take the form $\{-\mathcal{X} \}_k$, where $\x$ is an \xcoord\ on $\Gr(4,n)$
\item[$\bullet$] The symbol of $R^{(2)}_n$ can be expressed in terms of symbol letters drawn from the set of \acoords\ on $\Gr(4,n)$
\item[$\bullet$] The `classical tail' of $R^{(2)}_n$ (that is, the part of the amplitude that is left after accounting for the nonclassical component captured by $\delta_{2,2}$) can be expressed in terms of (products of) classical polylogarithms taking the form $\text{Li}_k(-\x)$, where $\x$ is again an \xcoord\ on $\Gr(4,n)$
\end{itemize}
Each of these statements represents a surprising (and independent) fact about $R^{(2)}_n$. \draftnote{-last one is independent b/c there exist trilogs that are cluster-A functions but not cluster-X functions; -first one }  
