...
We now turn to a more systematic exploration of subalgebra-constructible cluster polylogarithms. We restrict our attention to functions that can be defined on $\Gr(4,7) \simeq E_6$ and its subalgebras, as this space is complex enough to admit nontrivial functional embeddings but can be explored exhaustively since it is finite. The techniques we utilize can also be applied to infinite cluster algebras (see, for example, ~\cite{Golden:2014xqa}), but we leave exploration of this kind to future work~\cite{cluster_subalgebras_ii}. To focus our investigations, we will in particular look for further ways the nonclassical component of $R^{(2)}_{7}$ can be decomposed into its subalgebras, thus probing the extent to which $\mathcal{N}=4$ SYM theory knows about the rich subalgebra structure of $E_6$. In the remainder of this and the following sections, we therefore ignore purely classical polylogarithms. However, it is our hope that the full space of subalgebra-constructible cluster polylogarithms will prove to be of more general interest to physicists and mathematicians.
...
In practice, then, the existence (and uniqueness) of $f_{A_2}$ solves the problem of writing down weight-four cluster polylogarithms. 
...
We begin bootstrapping our way up to polylogarithms associated with larger algebras by determine the space of weight-four cluster polylogarithms associated with the $A_3$ cluster algebra. This is equivalent to asking: how many cluster polylogarithms can be constructed out of the $A_2$ subalgebras of $A_3$? 
...
\end{array} 
\end{equation}
This displays the total number of non-classical weight-four cluster polylogarithm functions associated with the $A_3$ cluster algebra with the given sign changes under the $\sigma$ and $\tau$ automorphisms. 
