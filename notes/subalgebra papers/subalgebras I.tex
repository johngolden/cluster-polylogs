...
\end{itemize}
It would be interesting to understand or provide some better motivation for the particular form of $f_{A_4}^{+-}$.

Having provided cluster algebraic motivations for certain propitious choices of $c_1$ and $c_2$, we now examine whether $f_{D_5}^{---}$ can be endowed with interesting behaviour under kinematic limits. In particular, we know that the nonclassical portion of $R^{(2)}_7$ must vanish under the $7\to6$ collinear limit, as $R^{(2)}_6$ is purely classical. It would be nice if each of the 14 $f_{D_5}^{---}$ functions vanished individually in this limit. 
...
\end{equation}
Remarkably, this is the same $f_{A_4}^{+-}$ function defined for the $D_5$ function in eq.~(\ref{eq:fa4-def}). We will discuss the implications of this in the next subsection, for now let us address the question of the behavior of $f_{A_5}^{--}$ in the $7\to6$ collinear limit.


\draftnote{kinematic limits}
