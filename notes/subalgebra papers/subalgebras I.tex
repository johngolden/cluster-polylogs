...
Cluster algebras come equipped with an automorphism group that maps the set of cluster coordinates (but not necessarily the set of clusters) back to itself. We introduce here only what we need to elucidate the automorphisms of the cluster algebras introduced in the last section. We refer the interested reader to~\cite{Chang:2015} for a more thorough mathematical introduction. 

The simplest example of a cluster automorphism is what we will call a direct automorphism. Let $\a$ be a cluster algebra equipped with a mutation rule $\mu(x_i, \bf{x})$ that mutates the cluster $\bf{x}$ on node $x_i$. Then,
\begin{quote}
...
\end{itemize}
\end{quote}
$A_2$ is also equipped with an indirect automorphism
\begin{equation}
...
The cluster algebra $D_4$ has automorphism group $D_4\times S_3$ (where the first factor is the dihedral group of order eight, and the second factor the symmetric group of order six). Each factor comes with a cyclic (direct automorphism) generator: 
\vspace{.1cm}
\begin{align}
  \sigma^{(4)}_{D_4}:\quad& 
  ...
  \sigma^{(3)}_{D_4}:\quad& 
    ...
    x_4\to \frac{1}{x_1} ,
\end{align}
where $\sigma^{(4)}_{D_4}$ has length four and is associated with the dihedral group, while $\sigma^{(3)}_{D_4}$ has length three and is associated with the symmetric group. Then there are two flip generators
\vspace{.1cm}
\begin{equation}
\begin{split}
  \tau_{D_4}:\quad& 
...
\end{split}  
\end{equation}
where $\tau_{D_4}$ is associated with the dihedral group and generates an indirect automorphism, while $\mathbb{Z}_2$ is associated with the symmetric group and generates a direct automorphism.

The cluster algebras on $D_{n>4}$,
\begin{equation}
    \begin{gathered}
    \begin{xy} 0;<1pt,0pt>:<0pt,-1pt>::
      (0,20) *+{x_1} ="1",
    ...
    \end{gathered},
\end{equation}
have automorphism group $D_n \times \mathbb{Z}_2$ (the first factor representing a dihedral group), with generators $\sigma_{D_n}$ (length $n$, direct), $\tau_{D_n}$ (length 2, indirect), and $\mathbb{Z}_{2,D_n}$ (length 2, direct). In the case of $D_5$, these generators can be chosen to be
\vspace{.1cm}
\begin{equation}
\begin{split}
  \sigma_{D_5}:\quad& 
