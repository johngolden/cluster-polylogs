...
We now turn to a more systematic exploration of the space of nonclassical cluster polylogarithms. We restrict our attention to functions that can be defined on $\Gr(4,7) \simeq E_6$ and its subalgebras, as this space is complex enough to give rise to an interesting space of functions (and, as will be seen, to admit nontrivial functional embeddings) yet can be explored exhaustively. The techniques we utilize can also be applied to infinite cluster algebras (see for instance~\cite{Golden:2014xqa}), but we leave exploration of this kind to future work~\cite{cluster_subalgebras_ii}. In the next section, we will take a top-down approach to look for novel ways in which the nonclassical component of $R^{(2)}_{7}$ can be decomposed into its subalgebras, thus probing the extent to which $\mathcal{N}=4$ SYM theory knows about the rich subalgebra structure of $E_6$. However, we present this bottom-up construction first in the hope that the full space of subalgebra-constructible cluster polylogarithms will be of general interest to physicists and mathematicians. We have tabulated the dimensions of spaces of weight-four cluster polylogarithms described in this section in appendix~\ref{appendix:cobrackets}, where we also tabulate the number of weight-four functions that have any nonzero cobracket component.

\subsection{\pdfeq{A_2} Functions as a Basis}

A remarkable (conjectured) property of $f_{A_2}$ is that it forms a complete basis for the $\delta_{2,2}$ component of any weight-four cluster polylogarithm. (This property can be constructively realized for all cluster polylogarithms defined on $E_6$ or its subalgebras, but has not been shown in general.) That is, given a weight-four cluster polylogarithm $F$ defined on a cluster algebra $\a$, there always exists some decomposition
...
In practice, then, the existence (and uniqueness) of $f_{A_2}$ solves the problem of writing down weight-four cluster polylogarithms. To illustrate this, let us construct all nonclassical $A_3$ cluster polylogarithms. This space could be computed by forming an ansatz of all possible cobracket components constructed out of cluster \xcoords, and then imposing the integrability condition~\eqref{eq:def-cobracket-integrability}. However, the decomposability conjecture~\eqref{eq:a2-decomp} reduces this computation to the simpler question: how many cluster polylogarithms can be constructed out of the $A_2$ subalgebras of $A_3$? 

Recall from section~\ref{sec:subalgebras_associahedra} that there are six $A_2$ subalgebras in $A_3$. There are multiple ways to think of these subalgebras---for instance, that they arise from the $\binom{6}{5}$ pentagons embeddable in a hexagon, or as the six pentagonal faces in the $A_3$ associahedron from figure~\ref{fig:a3-poly}. To label these subalgebras, we choose a representative cluster from each one. For instance, in terms of $A_3$ coordinates (defined by the initial seed $x_1 \to x_2 \to x_3$), we can choose the labels
...
\end{gathered}	
\end{equation}
as each of these seeds generates a distinct $A_2$ cluster algebra. Given these labels, we can construct an ansatz for all nonclassical $A_3$ functions out of $f_{A_2}$ functions evaluated on these subalgebras:
...
\begin{gathered}
	\sigma_{A_3}\big(f_{A_3} (x_1 \to x_2\to x_3) \big) = f_{A_3}(x_1 \to x_2\to x_3),\\ \\[-1.4em]
	\tau_{A_3}\big(f_{A_3} (x_1 \to x_2\to x_3) \big) = f_{A_3}(x_1 \to x_2\to x_3). 
\end{gathered}
\end{equation}
In practice this means we take the ansatz for $f_{A_3}$ in eq.~(\ref{eq:fa3-ansatz}) and attempt to find $c_i$ such that  \draftnote{check in new \xcoord\ conventions}
\begin{equation}
\begin{gathered}
...
\end{gathered}
\end{equation}
However, it turns out there is no collection of $c_i$ that solves both constraints. Conversely, allowing $f_{A_3}$ to pick up an overall minus sign when acted upon by $\tau_{A_3}$---that is, solving the constraints
\begin{equation}
\begin{gathered}
	\sigma_{A_3}\big(f_{A_3}(x_1\to x_2\to x_3)\big) = f_{A_3}(x_1\to x_2\to x_3),\\ \\[-1.4em]
	 \tau_{A_3}\big(f_{A_3}(x_1\to x_2\to x_3)\big) = -f_{A_3}(x_1\to x_2\to x_3),
...
\end{align}
where the superscripts on $f_{A_3}^{+-}$ indicate its behavior under $\sigma$ and $\tau$, respectively. (The superscript on $\sigma_{A_3}^i$ indicates how many times the operator $\sigma_{A_3}$ should be applied.) There are two remaining sign choices to check, $f_{A_3}^{-+}$ and $f_{A_3}^{--}$. We find only the trivial solution $f_{A_3}^{-+} = 0$, while		
\begin{equation}
	f_{A_3}^{--}(x_1\to x_2\to x_3) =\sum_{i=1}^6(-1)^i\sigma_{A_3}^i \big(f_{A_2}(x_1\to x_2)\big)
\end{equation}
is the unique $A_3$ function with this automorphism signature, up to an overall rescaling.
...
\end{array} 
\end{equation}
where each entry denotes the number of nonclassical weight-four cluster polylogarithms with a given automorphism signature, and the numbers in parentheses indicate how many of these functions additionally satisfy cobracket-level cluster adjacency. It is interesting to note that the automorphism signature $\sigma^+\tau^- $ admits at least one solution for each of these $A_n$, and in particular that it admits the only nontrivial solution in both $A_2$ and $A_4$. 

This procedure gets only slightly more complicated in the $D_n$ algebras, due to their larger automorphism groups. As discussed in section~\ref{sec:automorphisms}, $D_4$ has the automorphism group $D_4\times S_3$ (the first factor being the dihedral group of order eight), with two cyclic generators $\smash{\sigma_{D_4}^{(4)}}$ and $\smash{\sigma_{D_4}^{(3)}}$ (corresponding to the $D_4$ and $S_3$, respectively) and the $D_4$ flip $\tau_{D_4}$ and $S_3$ flip whose generator we denote as $\mathbb{Z}_{2,D_4}$. This gives rise to 16 possible automorphism signatures, which we impose on a general ansatz of $f_{A_2}$ functions evaluated on the 36 distinct $A_2$ subalgebras of $D_4$. In the same conventions as table~\eqref{eq:an-func-counts}, we find:
\begin{equation}
\begin{tabular}{ c | c | c |}
...
\cline{1-3} $\mathbb{Z}_2^-$ & 1 (0) & \multicolumn{1}{c}{0} 
\end{tabular} 
\hspace{.6cm}
\begin{tabular}{ c | c | c |}
\multicolumn{1}{c}{} &\multicolumn{2}{c}{$\underline{\ \sigma_4^+ \tau_4^- \ }$} \\[-1em]
...
\multicolumn{1}{c}{} & \multicolumn{1}{c}{$\sigma_{3}^+$} & \multicolumn{1}{c}{$\sigma_{3}^-$} \\[-1em]
 & & \multicolumn{1}{c}{} \\
\cline{1-3} $\mathbb{Z}_2^+$ & 2 (0) & \multicolumn{1}{c}{0} \\
\cline{1-3} $\mathbb{Z}_2^-$ & 0 & \multicolumn{1}{c}{0}
\end{tabular}
\hspace{.6cm}
\begin{tabular}{ c | c | c |}
...
\multicolumn{1}{c}{} & \multicolumn{1}{c}{$\sigma_{3}^+$} & \multicolumn{1}{c}{$\sigma_{3}^-$} \\[-1em]
 & & \multicolumn{1}{c}{} \\
\cline{1-3} $\mathbb{Z}_2^+$ & 1 (0) & \multicolumn{1}{c}{0} \\
\cline{1-3} $\mathbb{Z}_2^-$ & 1 (1) & \multicolumn{1}{c}{0} 
\end{tabular}
\hspace{.6cm}
\begin{tabular}{ c | c | c |}
\multicolumn{1}{c}{} &\multicolumn{2}{c}{$\underline{\ \sigma_4^- \tau_4^- \ }$} \\[-1em]
...
\vspace{.2cm}
\end{equation}
Here we again see that the space of functions satisfying automorphisms is remarkably constrained. No functions flip sign under $\sigma_3$, and only the signature $\sigma_4^+\tau_4^-\sigma_3^+\mathbb{Z}_2^+$ gives rise to more than one solution.


$D_5$ is less symmetric than $D_4$, with only three generators in its automorphism group $D_5 \times \mathbb{Z}_2$ (where again, the first factor represents the dihedral group, not the Dynkin diagram). This means there are 8 possible automorphism signatures, which we impose on a general ansatz formed from the 125 $A_2$ subalgebras in $D_5$, finding:
...
\end{tabular} 
\end{equation}
Interestingly, no nontrivial solutions exist that have a different signature under $\sigma$ and $\mathbb{Z}_2$.
...
\end{tabular} 
\end{equation}
It is especially surprising that there are no solutions that are odd under $\sigma$. At this point we also recall that $R^{(2)}_7$ is a cluster polylogarithm on $E_6\simeq \Gr(4,7)$ that is invariant under the full automorphism group (which combines the dihedral group corresponding to Bose symmetry with parity). Therefore the remainder function must be some linear combination of the 14 degrees of freedom in $f_{E_6}^{+++}$. 

We pause at this point to emphasize that this method has been verified to generate all nonclassical functions on each of the finite cluster algebras considered above, and its completeness only becomes conjectural on infinite cluster algebras. While it is clearly not possible to form an ansatz out of all $A_2$ subalgebras in an infinite cluster algebra such as $\Gr(4,8)$, this method can still be used to generate all nonclassical polylogarithms that are constructible out of (the automorphic completion of) any finite set of $A_2$ subalgebras. This proves sufficient in the case of the two-loop remainder function to all $n$~\cite{Golden:2014xqa}, and may prove sufficient in other cases of physical interest. 

\subsection{Nested Cluster Polylogarithms}

Given the striking utility of linear combinations of $f_{A_2}$ functions, it is natural to ask whether other (even more special) spaces of functions are constructible out of cluster polylogarithms defined on larger algebras. In fact, this has already been shown to be the case, since the nonclassical part of all two-loop MHV amplitudes can be constructed out of $f_{A_3}^{--}$~\cite{Golden:2014xqa}. This gives rise to an aesthetically appealing nested set of cluster polylogarithms, since $f_{A_3}^{--}$ is itself constructible out of $f_{A_2}$. More generally, in the functions constructed in section~\ref{sec:all_nonclassical_polylogs_gr47} there will be many instances of $A_2$ subalgebras joining together into larger subalgebras, and it remains an open question how intrinsically interesting such nested constructibility might be. 

The procedure for constructing such spaces clearly proceeds just as in the case of $f_{A_2}$. However, there are now multiple spaces one can construct, since there are multiple cluster polylogarithms associated with larger cluster algebras. We leave the exploration of compositely-constructible spaces to future work. For now, we just consider the example of functions constructible out of $f_{A_3}^{--}$, since all $R_n^{(2)}$ are believed to be in this class.

Although there are 6 and 9 independent $A_3^{--}$\!-constructible functions in $A_4$ and $D_4$, they cannot be put into combinations that respect the automorphisms of either algebra. In $A_5$ we find 
\begin{equation}\label{eq:a3-a5-func-counts}
\begin{array}{ l | c | c | c | c }      
  & \sigma^+\tau^+ & \sigma^+\tau^- & \sigma^-\tau^+ & \sigma^-\tau^- \\
  \hline
  A_3^{--}\subset A_5 & 1 \ (1) & 1 \ (1) & 0 & 3 \ (3)
\end{array} 
\end{equation}
while in $D_5$ we have
\begin{equation}
\begin{tabular}{c | c |}
\multicolumn{2}{c}{$\underline{\ \sigma^+ \tau^+ \ }$} \\[-1em]
\multicolumn{1}{c}{} & \multicolumn{1}{c}{} \\
...
\end{tabular} 
\end{equation}
Since $f_{A_3}^{--}$ satisfies cobracket-level cluster adjacency, all functions constructed out of it also have this property. 

\section{Subalgebra-Constructibility and \pdfeq{R^{(2)}_7}} \label{sec:r27-sub-constructibility}

Systematic study of subalgebra-constructible polylogs...this space is complex enough to admit nontrivial functional embeddings yet can be explored exhaustively
...
\begin{center}
\begin{tabular}{ | c | c | c | c | c |  c |}
\multicolumn{1}{c}{\multirow{2}{*}{Type}} & \multicolumn{1}{c}{\multirow{2}{*}{Nonclassical Cobrackets}} & \multicolumn{4}{c}{Automorphic} \\
...
\begin{center}
\begin{tabular}{ | c | c | c | c | c |  c |}
\multicolumn{1}{c}{\multirow{2}{*}{Type}} & \multicolumn{1}{c}{\multirow{2}{*}{Integrable Cobrackets}} & \multicolumn{4}{c}{Automorphic} \\
\multicolumn{1}{c}{} & \multicolumn{1}{c}{} & \multicolumn{1}{c}{$\sigma^+ \tau^+$} & \multicolumn{1}{c}{$\sigma^+ \tau^-$} & \multicolumn{1}{c}{$\sigma^- \tau^+$} & \multicolumn{1}{c}{$\sigma^- \tau^-$} \\
