...
Cluster algebras were first introduced by Fomin and Zelevinsky \cite{1021.16017} as a tool for identifying when algebraic varieties come equipped with a natural notion of positivity, and what quantities determine this positivity. As a consequence, they naturally appear in the study of the positive Grassmannian $\Gr_{+}(k,n)$, i.e.~the space of $k\times n$ matrices where all ordered $k\times k$ minors are positive, which in turn encodes the all-orders integrands of planar ${\cal N} = 4$ SYM~\cite{ArkaniHamed:2012nw}. 

As an example, a basic question one can ask is: how many matrix minors are sufficient to specify a point in $\Gr_+(k,n)$? In other words, given a $k \times n$ matrix $M$, how many minors of $M$ do we have to calculate to know if $M \in \Gr_+(k,n)$? The reason that this is an interesting question is that the minors are not all independent; they satisfy identities known as Pl\"ucker relations,
\begin{equation}
  \label{eq:plucker-rel}
  \ket{abI} \ket{cdI} = \ket{acI} \ket{bdI} + \ket{adI}\ket{bcI},
\end{equation}
where each Pl\"ucker coordinate $\ket{i_1,\ldots,i_k}$ corresponds to the minor of columns $i_1, \ldots,i_k$, and $I$ is a multi-index with $k-2$ entries.

Let us first consider this question in the case of $\Gr_+(2,5)$. The 5 cyclically adjacent minors $\ket{12}$, $\ket{23}$, $\ket{34}$, $\ket{45}$, and $\ket{15}$ are not related by Pl\"ucker relations, and so each give an independent positivity constraint. This leaves 5 non-adjacent minors that might need to be checked. However, the Pl\"ucker relations
\begin{equation}
\begin{split}
...
\end{split}	 	
\end{equation} 
allow us to rewrite 3 of these minors in terms of the remaining 7. The remaining non-adjacent minors, $\ket{13}$ and $\ket{14}$, turn out to be independent and so their positivity provides two new constraints. Thus, checking the positivity of 7 minors suffices to answer whether or not a given $2 \times 5$ matrix is in $\Gr_+(2,5)$. However, it is easy to convince yourself that we could not have chosen any 7 minors, as it is not the case that any 3 minors can be rewritten in terms of the remaining 7. To address this question in full generality, we therefore want a general way of determining {\it which} sets of minors prove sufficient to determine positivity.

To see how cluster algebras address this question, consider the following triangulation of the pentagon:
\begin{equation}
\begin{tikzpicture}
  \drawLabeledPentagon
