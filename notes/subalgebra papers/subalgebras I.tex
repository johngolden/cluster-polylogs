...
\begin{align}
  ...
  &\ \ \vdots \nn
\end{align}
\draftnote{this was wrong, I needed to invert coordinates. Check and delete draftnote} The series then repeats, with all arrows reversed. Note that (specifically in the case of $A_2$), if we label these $\x$-coordinates by
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
(Note that evaluating the above expression for $k=2$ will not directly give~\eqref{eq:g2n-seed}; the two are equivalent after a cyclic rotation and dihedral flip of the $n$-gon.)\draftnote{I find that, rather, it's a cyclic rotation and then just flipping the arrows on \eqref{eq:g2n-seed}. Do you agree?} The cluster algebra on $\Gr(k,n)$ is therefore of rank $(n-k-1)(k-1)$, i.e.~the number of mutable nodes in~\eqref{eq:gkn-seed}. 
...
Note that the notation introduced in~\eqref{eq:D5_in_E6_sum} and~\eqref{eq:A5_in_E6_sum} is subtly different than that used previously in this paper, such as~\eqref{eq:e6-symmetric-sum}. The two notations are, in practice, equivalent up to an overall rescaling, for example 
\begin{equation}
	\sum_{E_6^{+++}} f_{D_5}(x_i \to \ldots) = 2 \sum_{D_5\subset E_6} f_{D_5}(x_i \to \ldots).
\end{equation}
We will adopt the latter notation in the case where it is useful to interpret the sum as literally going over the subalgebras of a single larger algebra. The downside of this representation is that it obfuscates the ways in which the overall sum respects indirect automorphisms of the larger algebra. We use the former notation where it is instructive to make the invariance (up to a sign) under all automorphisms explicit, while sacrificing the direct interpretation as a sum over subalgebras of a single algebra (and must instead be technically interpreted as a sum over subalgebras of the original algebra as well as subalgebras of any ``flipped'' versions generated by indirect automorphisms). But again let us emphasize that these two representations are essentially equivalent, and that the resulting functions can always be thought of simultaneously as being sums over subalgebras of a single larger algebra while also respecting indirect automorphisms. 
...
The only subalgebra of $\Gr(4,7)$ we have not had reason to touch on yet is $D_4$. Interestingly, there exists exactly one $D_4$-constructible function in $E_6$ that has signature $\sigma_{E_6}^+ \tau_{E_6}^+ \mathbb{Z}_{2,E_6}^+$ and respects cobracket-level cluster adjacency. This function fails to vanish (or be well-defined) in the $7\to6$ collinear limit, however, and so cannot be directly related to $R_{7}^{(2)}\!$. Still, the uniqueness of this function is intriguing and it will perhaps be useful in other physical contexts.
>>>>>>> 2a1d6f08363faad2d21d245655dc075f1e1e9f43
...
\caption{The number of weight-four cluster polylogarithms on various finite cluster algebras that have nonzero cobrackets, prior to consideration of their automorphism group and after requiring specific automorphism signatures. The number of polylogarithms that can also be made to satisfy cobracket-level cluster adjacency is given in parentheses.}
