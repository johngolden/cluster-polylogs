...
\abstract{We review various aspects of cluster algebras and the ways in which they appear in the study of loop-level amplitudes in planar ${\cal N} = 4$ supersymmetric Yang-Mills theory. In particular, we highlight the different forms of cluster-algebraic structure that appear in the two-loop MHV amplitudes of this theory at the function, symbol, and Lie cobracket level, and review how the `nonclassical' part of these amplitudes can be decomposed into specific functions evaluated on the $A_2$ or $A_3$ subalgebras of $\Gr(4,n)$. We then extend the latter line of inquiry by searching for other subalgebras over which these amplitudes can be decomposed. We focus on the case of seven-particle kinematics, where we show that the nonclassical part of the two-loop MHV amplitude is also constructible out of individual functions evaluated on the $A_4$, $A_5$, and $D_5$ subalgebras of $\Gr(4,7)$. Unlike the $A_3$ decompositions of this amplitude, these }
...
\begin{align}
\sum_{i=0}^6\sum_{j=0}^1\sum_{k=0}^1 \ \tau_{E_6}^k \circ \mathbb{Z}_{2,E_6}^j \circ \sigma_{E_6}^i &\Bigg( \delta_{2,2}\Bigg[ 2 f_{A_3}^{--}\left(x_1 \to x_2 (1 + x_3)\to \frac{x_3 x_4 x_5}{1 + x_{5,3}} \right)  \nonumber \\
&- f_{A_3}^{--}\left(x_1\to \frac{x_2 (1 + x_{6,5,3,4})}{1 + x_{6,5}}\to \frac{x_3 x_5}{(1 + x_5) (1 + x_{6,5,3})}\right)  \\ 
&- f_{A_3}^{--}\left(x_1\to x_2 (1 + x_3)\to \frac{x_3 x_4 x_5 x_6}{1 + x_{6,5,3}} \right) \Bigg] \Bigg) = 0. \nonumber
\end{align}
...
\begin{align}
	...
	&\left(-\frac{1}{2}-c_1\right) f_{A_2}\left(\frac{x_1 x_2}{1+x_1}\to x_3 \left(1+x_4\right)\right) \bigg] \label{eq:fa5-init}
\end{align}
...
Just as with the the $D_5$ decomposition, we can tune $c_1$ to make $f_{A_5}^{--}$ itself either $A_3$- or $A_4$-constructible. However, this time the $A_3$ decomposition can only be carried out in terms of the function $f_{A_3}^{+-}$. Choosing $c_1 = -\frac{1}{2}$, we have
\begin{equation}
	f_{A_3\subset A_5}^{--} = -\frac{1}{160}\sum_{A_5^{--}} f_{A_3}^{+-}\left(x_2\to x_3(1+x_4)\to \frac{x_4 x_5}{1+x_4}\right),
\end{equation}
where the overall coefficient of $-\frac{1}{160}$ results from overcounting due to the symmetric sum. If we instead choose $c_1 = -1$, we get the decomposition
\begin{equation}
	f_{A_4\subset A_5}^{--} = -\frac{1}{20} \sum_{A_5^{--}} f_{A_4}^{+-}(x_1\to x_2 \to x_3 \to x_4)
\end{equation}
in terms of the \emph{same} $f_{A_4}^{+-}$ function that $f_{D_5}^{---}$ was decomposable into. 

Before discussing the remarkable appearance of $f_{A_4}^{+-}$ in both the $D_5$ and the $A_5$ decomposition of $\smash{R_7^{(2)}}$\!, let us address the question of $f_{A_5}^{--}$'s collinear limits. In this case, all 7 instances of $f_{A_5}^{--}$ on $E_6$ vanishing at the nonclassical level in the $7\to6$ collinear limit, for all values of $c_1$. This makes the collinear behavior of $\smash{R^{(2)}_7}$ considerably cleaner in the $A_5$ decomposition than was the case for the $D_5$ decomposition. 

\subsection{$R_7^{(2)}$ from its Subalgebra Structure}
...
\item other kinematic limits that assign the $D_5$, $A_5$, or $A_4$ function a clear physical interpretation
\end{enumerate}
