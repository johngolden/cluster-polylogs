...
\subsection{\pdfeq{A_2} Functions as a Basis for Nonclassical Cluster Polylogarithms}
...
\begin{gathered}
	x_1 \to x_2, \qquad \qquad
	x_2 \to x_3, \qquad \qquad
	\frac{x_2}{1+x_{1,2}}\to \frac{\left(1+x_1\right) x_3}{1+x_{1,2,3}},\\ \\
	\frac{x_1 x_2}{1+x_1}\to x_3,\qquad \qquad
	x_1 \left(1+x_2\right)\to \frac{x_2 x_3}{1+x_2},\qquad \qquad
...
\begin{equation}
\begin{gathered}
	f_{A_3}(\frac{x_2}{1+ x_{1,2}}\to \frac{x_3(1+x_1)}{1+x_{1,2,3}}\to \frac{1+x_{1,2}}{x_1x_2x_3}) = f_{A_3}(x_1\to x_2\to x_3),\\ \\[-1.4em]
...
\begin{equation}
\begin{gathered}
	\sigma_{A_3}(f_{A_3}(x_1\to x_2\to x_3)) = f_{A_3}(x_1\to x_2\to x_3),\\ \\[-1.4em]
...
\begin{equation}
	c_i = 1
\end{equation}
(which can always be rescaled by an overall constant). We label this particular solution by 
\begin{align}
...
\end{align}
where the superscripts on $f_{A_3}^{+-}$ indicate its behavior under $\sigma$ and $\tau$, respectively. (Whereas the superscript on $\sigma_{A_3}^i$ indicates how many times the operator $\sigma_{A_3}$ should be applied.) Similarly, based on the lack of a solution for eq.~(\ref{eq:fA3++}) we say that $f_{A_3}^{++}(x_1\to x_2\to x_3) = 0$. There are two remaining sign choices to check, $f_{A_3}^{-+}$ and $f_{A_3}^{--}$. We find $f_{A_3}^{-+} = 0$, while		
\begin{equation}
...
\end{array} 
\end{equation}
where the entries denote the number of non-classical weight-four $A_3$ cluster polylogarithms with a given set of automorphism properties. 
...
\begin{center}
\begin{tabular}{ | c | c | c | c | c |  c |}
\multicolumn{1}{c}{\multirow{2}{*}{Type}} & \multicolumn{1}{c}{\multirow{2}{*}{Cobracket-Adjacent}} & \multicolumn{4}{c}{Automorphic} \\
...
\hline \(A_2\) & 6 & 1 & 1 & 0 & 0 \\ 
\hline \(A_3\) & 21  & 2 & 2 & 0 & 3 \\ 
\hline \(A_4\) & 56 & 4 & 4 & 0 & 0 \\ 
\hline \(D_4\) & 86   \\ 
\hline \(A_5\) & 126 & 8 & 8 & 5 & 11 \\ 
\hline \(D_5\) & -   \\ 
\hline \(E_6\) & -  & - & - & - & -  \\ 
\hline
\end{tabular} 
