...
A growing body of evidence suggests that cluster algebras (or their generalizations) have a central role to play in multi-loop scattering amplitudes in the planar limit of ${\cal N} = 4$ supersymmetric Yang-Mills (SYM) theory. All known loop-level amplitudes in this theory are expressible in terms of cluster coordinates~\cite{Golden:2013xva,Golden:2013lha,Golden:2014xqa,Golden:2014pua,Dixon:2013eka,Dixon:2014voa,Dixon:2014iba,Drummond:2014ffa,Dixon:2015iva,Caron-Huot:2016owq,Dixon:2016nkn,Drummond:2017ssj}, as are certain classes of Feynman integrals~\cite{Drummond:2010cz,Drummond:2017ssj,Bourjaily:2018aeq,Henn:2018cdp}, some of which have been computed to all loop orders~\cite{Caron-Huot:2018dsv}. Even more strikingly, it has been conjectured that cluster coordinates only appear in adjacent symbol entries of these (appropriately normalized) amplitudes and integrals when these coordinates also appear together in a cluster of Gr(4,$n$)~\cite{Drummond:2017ssj}. This observation is also borne out by all known amplitudes~\cite{all_orders_adjacency}. Thus, cluster algebras seem to combinatorially realize the analytic structure of these amplitides and integrals, encoding their locality in a non-obvious way.  

The fact that cluster algebras appear in this context is not totally surprising, since they are dual to plabic graphs, which describe the integrands of this theory to all orders~\cite{ArkaniHamed:2012nw}. However, there is no known \emph{a priori} reason these structures should survive integration---and indeed, it has recently been appreciated that algebraic roots appear in Feynman integrals contributing to eight- and higher-particle amplitudes~\cite{Prlina:2017azl,Bourjaily:2018aeq,Henn:2018cdp}. This obfuscates the connection between amplitudes and cluster algebras (which only involve rational transformations), as does the eventual appearance of functions beyond polylogarithms in this theory~\cite{}. Both complications point to the need for some generalization of cluster algebras (or of the role cluster coordinates play in iterated integrals) if analogous algebraic structure is to be found at higher loops and particle multiplicities.
...
The remainder of this paper is organized as follows. In section~\ref{sec:brief_intro} we provide a self-contained introduction to cluster algebras and the ways in which they appear in the planar limit of ${\cal N}=4$ SYM theory. This section is intended to fill a pedagogical gap in the physics literature, and can be skipped by those who are familiar with recent developments at the intersection of these topics. In section~\ref{sec:cluster_polylog_MHV_review} we 
