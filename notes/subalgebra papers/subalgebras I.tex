...
Given the striking utility of linear combinations of $f_{A_2}$ functions, it is natural to ask whether even more special classes of functions can be constructed out of cluster polylogarithms defined on larger algebras. In fact, this has already been shown to be the case, since the nonclassical part of all two-loop MHV amplitudes can be constructed out of $f_{A_3}^{--}$~\cite{Golden:2014xqa}. This gives rise to an aesthetically appealing nested set of cluster polylogarithms, since $f_{A_3}^{--}$ is itself constructible out of $f_{A_2}$. More generally, in the functions constructed in section~\ref{sec:all_nonclassical_polylogs_gr47} there will be many instances of $A_2$ subalgebras joining together into larger subalgebras, and it remains an open question how intrinsically interesting such nested constructibility might be. 

The procedure for constructing such spaces clearly proceeds just as in the case of $f_{A_2}$. However, there are now multiple spaces one can consider constructing since in general there are multiple cluster polylogarithms associated with larger cluster algebras. To simplify matters, we illustrate how this works by restricting our basis to just the function $f_{A_3}^{--}$.
