...
To connect this triangulation to points in $\Gr_+(2,5)$, we associate the line connecting points $i$ and $j$ with the Pl\"ucker coordinate $\ket{ij}$. If we assign this line length $\ket{ij}$, the resulting pentagon is cyclic due to Ptolemy's theorem; more generally, all cyclic $n$-gons represent a point in $\Gr_+(2,n)$. Note that the length of the three diagonals that are not present in this triangulation are determined by the length of the seven lines that are (including edges). This makes clear why the seven minors identified above are sufficient for determining whether a matrix is in $\Gr_+(2,5)$. Moreover, from the first relation in~\eqref{eq:plucker} we see that we could have chosen to check the positivity of $\ket{24}$ instead of $\ket{13}$. This corresponds to choosing a different triangulation, which we get to by trading the latter diagonal for the former:
\begin{equation}
\begin{tikzpicture}
  \drawLabeledPentagon
...
\end{tikzpicture}  \ .
\end{equation}
More generally, we can pick any quadrilateral and flip the diagonal it contains to generate a different triangulation. Repeatedly performing these flips generates all possible triangulations of the pentagon, as can be seen in figure~\ref{fig:pent_triangulations}. Each triangulation provides a set of edges/minors whose positivity ensures that a matrix is in $\Gr_+(2,5)$.

Cluster algebras analagously address the same question for a larger set of algebraic varieties by considering {\it clusters} that are generated by an operation called {\it mutation}, just as triangulations of the pentagon are generated by flipping the diagonals of quadrilaterals. Let us now turn to the definition of these more general objects.
