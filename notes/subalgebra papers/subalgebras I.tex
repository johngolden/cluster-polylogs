...
Just as with $f_{D_5}^{---}$, we can tune the free parameter in $f_{A_5}^{--}$ to make it either $A_3$- or $A_4$-constructible. Choosing $c_1 = -\frac{1}{2}$, it becomes $A_3$-constructible, but this time in terms of the function $f_{A_3}^{+-}$. Specifically, we find
\begin{equation}
	f_{A_3\subset A_5}^{--} = -\frac{1}{160}\sum_{A_5^{--}} f_{A_3}^{+-}\left(x_2\to x_3(1+x_4)\to \frac{x_4 x_5}{1+x_4}\right),
\end{equation}
where the larger denominator results from the fact $f_{A_3}^{+-}$ only has four distinct automorphic images in this sum (of course, $f_{A_3}^{+-}$ evaluates to a larger number of distinct functions on the other $A_3$ subalgebras of $A_5$). If we instead choose $c_1 = -1$, $f_{A_5}^{--}$ becomes decomposable into its $A_4$ subalgebras. Similar to $D_5$, there is only a single orbit of $A_4$ subalgebras in $A_5$, implying that this decomposition must take the form 
\begin{equation}
...
\end{equation}
in which the \emph{same} $\smash{f_{A_4}^{+-}}$ function appears that was found in the decomposition of $\smash{f_{D_5}^{---}}$. 

Before discussing the remarkable appearance of the same function $A_4$ function in the $D_5$ and $A_5$ decompositions of $\smash{R_7^{(2)}}$\!, let us address the question of $f_{A_5}^{--}$'s collinear limits. In this case, all 7 instances of $f_{A_5}^{--}$ on $E_6$ vanishing at the nonclassical level in the $7\to6$ collinear limit, for all values of $c_1$. This makes the collinear behavior of $\smash{R^{(2)}_7}$ considerably cleaner in the $A_5$ decomposition than was the case for the $D_5$ decomposition. 
...
It is interesting to note that the only subalgebra for which there is no nonclassical decomposition is $D_4$.

\begin{enumerate}
\item other kinematic limits that assign the $D_5$, $A_5$, or $A_4$ function a clear physical interpretation (note that singular limits can be considered, but only the finite part will be visible)
\item while the six-point remainder function is too simple to have any nonclassical structure, it almost seems like the seven-point remainder function is engineered to have the most interesting nonclassical structure possible
\item but what does this subalgebra structure mean physically? Would be nice to find kinematic limits that isolate these different functions
\item there is no $D_4$ decomposition (and it's interesting to note that $D_5$ is the subalgebra with no Grassmannian interpretation, not $D_4$); more generally, why do certain subalgebra chains produce valid cluster polylogarithms while others do not?
\item discuss geometric aspects of this picture
\end{enumerate}


\section{Conclusion}

We have explored the space of nonclassical cluster polylogarithms on $\Gr(4,7)$ and its subalgebras, and have shown that---while this space is relatively small---it admits a natural geometric construction that gives rise to interesting sequences of nested functional embeddings. The seven-particle two-loop MHV amplitude seems almost specially engineered to take advantage of this subalgebra structure, insofar as the nonclassical part of this amplitude can be decomposed into cluster polylogarithms defined on almost every type of subalgebra appearing in $\Gr(4,7)$, excluding only $D_4$. The decompositions associated with the corank-one subalgebras $D_5$ and $A_5$ are especially canonical, as are the further decompositions of the associated $D_5$ and $A_5$ cluster polylogarithms into their $A_4$ subalgebras, as the form of each of these decompositions is uniquely dictated by the automorphism group of the parent algebra. These decompositions identify new cluster polylogarithms of special physical interest, supplementing the $A_2$ and $A_3$ cluster polylogarithms previously identified in~\cite{Golden:2014xqa}. It is natural to ask whether higher-point MHV amplitudes can also be decomposed into these new cluster polylogarithms, as is known to be the case for the $A_2$ and $A_3$ functions; we will take one step towards addressing this question in a forthcoming paper~\cite{cluster_subalgebras_ii}.
...
%coaction (in the guise of the symbol) can be thought of as breaking down polylogarithms into (a sum over) a sequence of genus zero curves associated with the zero locus of the denominator in their integration kernel~\eqref{eq:multiple_polylogs}, higher-genus and higher-dimensional varieties also generically appear in the integration contours of amplitudes in generic quantum field theories (including planar ${\cal N} = 4$ sYM theory~\cite{}) and will thus play some role in .
...
\begin{tikzpicture}[overlay, remember picture,decoration={markings,mark=at position .99 with {\arrow[scale=1.4,>=stealth]{>}}}]
    ...
    \draw [dashed,rounded corners=1cm] ([yshift=1cm,xshift=-1.3cm]{pic cs:e6topLeft2}) rectangle ([yshift=-.6cm,xshift=1cm]{pic cs:e6bottomRight2}) ;
...
\begin{tikzpicture}[overlay, remember picture,decoration={markings,mark=at position .99 with {\arrow[scale=1.4,>=stealth]{>}}}]
    ...
    \draw [dashed,rounded corners=1cm] ([yshift=1cm,xshift=-1.5cm]{pic cs:e6topLeft3}) rectangle ([yshift=-.6cm,xshift=1cm]{pic cs:e6bottomRight3}) ;
