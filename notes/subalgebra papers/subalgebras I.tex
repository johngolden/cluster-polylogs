...
The BDS ansatz captures the infrared structure of planar ${\cal N} = 4$ SYM to all orders~\cite{Bern:2005iz}. In four- and five-particle kinematics it also completely accounts for the finite part of the amplitude, while for six or more particles it must be corrected by a finite dual-conformally invariant function~\cite{Drummond:2007au,Bern:2008ap,Drummond:2008aq}. In the case of the MHV amplitude, this correction is often computed in the form of the $n$-particle remainder function $R_n$, defined by
\begin{equation}
...
\end{equation}
where $g^2 = \frac{g_{\text{YM}}^2 N_c}{16 \pi^2}$. We have here used the fact that $R_n^{(1)} = 0$, since the BDS ansatz encodes the complete one loop MHV amplitude at all $n$. 
...
The space of generalized polylogarithms defined by~\eqref{eq:multiple_polylogs} is colossally overcomplete. This is both because $a_i$ and $z$ are allowed to be arbitrarily complicated algebraic functions, and because these functions satisfy a shuffle and stuffle algebra. Their shuffle algebra represents the fact that unordered integrations can be triangulated into a sum over iterated integrals~\cite{Duhr:2011zq,Duhr:2014woa}. In general, this means that when two polylogarithms share an argument $z$, their product can be re-expressed as the sum of functions 
...
\end{equation}
where $\Sigma(k_1,k_2)$ denotes the set of all shuffles between the sets of integers $\{1,\dots,k_1\}$ and $\{k_1+1,\dots,k_1+k_2\}$ (that is, all ways of interleaving these two sets such that the ordering within each original set is maintained). The stuffle algebra naturally arises when generalized polylogarithms are re-expressed as infinite sums, and represents the freedom to split up unordered summation indices into nested sums where these indices are ordered; see~\cite{Duhr:2014woa} for more about these sum representations.

This overcompleteness gives rise to a rich space of identities. This can already be seen at the level of classical polylogarithms, which correspond to the instances
\begin{equation}
...
Fortunately, \emph{all} identities between polylogarithms are (conjecturally) generated by shuffle and stuffle relations~\cite{2011arXiv1102.1312B,2015arXiv151206409B}, which are trivialized by the symbol map~\cite{}. The symbol of a polylogarithm can be defined iteratively in terms of total derivatives. For instance, taking the total derivative of~\eqref{eq:multiple_polylogs}, we have
...
The symbol also captures the analytic structure of polylogarithms, insofar as it encodes their (iterated) discontinuity structure. Namely, the branch cut structure of a poylogarithm is completely dictated by the symbol letters appearing in its first entry, when the indices $a_i$ and argument $z$ are generic. These functions have nonzero monodromy when analytically continued around points where these symbol letters vanish or become infinite (sharing the monodromies of logarithms with these arguments), and are analytic everywhere else~\cite{Duhr:2012fh}. This monodromy can itself have branch cuts that the original function did not have, as encoded in the second entry of the symbol, \emph{ad infinitum}. For special values of $a_i$ and $z$, some of this information is lost due to the fact that higher-weight transcendental constants (such as Riemann $\zeta$ values) are in the kernel of the symbol~\cite{Gonch}. However, this information is retained by the coaction~\cite{Brown:2011ik}, and can be recovered by specifying the integration constant at a kinematic point. We defer further consideration of these transcendental constants to a follow-up paper~\cite{}.
...
\end{equation}
where $\rho(s_1) \equiv s_1$~\cite{Golden:2014xqa}. This operator projects onto the component of a symbol that cannot be written as a product of lower-weight polylogarithms (for instance, via the shuffle relations~\eqref{eq:shuffle_relation}). This operation can be lifted to a projection on functions, up to terms proportional to transcendental constants; since we will not be concerned with these terms in what follows, we abuse notation by applying $\rho$ directly to functions. The cobracket $\delta$ of a weight $k$ polylogarithm $f$ can then be calculated as
\begin{equation}
\delta(f) \equiv \sum_{i=1}^k (\rho_i \wedge \rho_{k-i})\rho(f),
\end{equation}
where this notation indicates that the projection operator $\rho$ is first applied to $f$, after which each term in the symbol is partitioned into a wedge product of its first $i$ and last $k-i$ entries, and the projection operator is applied to the entries in this wedge product separately. (Equivalently, the wedge product can be computed using the `weight $i,k-i$' component of the coproduct.)
