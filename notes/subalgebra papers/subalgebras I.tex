...
\end{equation}
where $g^2 = \frac{g_{\text{YM}}^2 N_c}{16 \pi^2}$. We have here used the fact that $R_n^{(1)} = 0$, since the BDS ansatz accounts for the complete one loop MHV amplitude for all $n$. 
...
In addition to being generalized polylogarithms, loop-level contributions to the remainder function exhibit a great deal of cluster-algebraic structure. For this reason, they were christened `cluster polylogarithms' in~\cite{Golden:2014xqa}, where this denoted the fact that their symbol is naturally expressible in terms of cluster \acoords, while their Lie cobracket is naturally expressible in terms of cluster \xcoords\ (in a way that will be made precise below). We propose that this definition be refined to include only functions that respect the automorphism group and the cluster adjacency principle. The rest of this section is devoted to making these properties precise, for which purpose we first describe the motivic structure of polylogarithms. 

\subsection{The Symbol and Cobracket} \label{sec:coproduct}

The space of generalized polylogarithms defined by~\eqref{eq:multiple_polylogs} is exceedingly overcomplete. This is because $a_i$ and $z$ are allowed to be arbitrarily complicated algebraic functions, making it possible to express a given polylogarithm multiple ways. This can already be seen at the level of classical polylogarithms, which correspond to the cases
\begin{equation}
...
In addition to the symbol, polylogarithms come equipped with a Lie cobracket structure $\delta$~\cite{Golden:2013xva}. It is calculated by
\begin{equation}
\delta(f) \equiv (\rho \circ \Delta \circ \rho)(f)
\end{equation}
