...
The only subalgebra of $\Gr(4,7)$ we have not had reason to touch on yet is $D_4$. There are in fact 4 $D_4$-constructible functions in $E_6$ that have signature $\sigma_{E_6}^+ \tau_{E_6}^+ \mathbb{Z}_{2,E_6}^+$. However, none of these functions can be put in a form that respects cobracket-level cluster adjacency, precluding any $D_4$ decomposition of $R_{7}^{(2)}\!$. At least at the nonclassical level, this indicates an incompatibility between the automorphism groups of $D_4$ and $A_3$ when respectively embedded in $E_6$.
