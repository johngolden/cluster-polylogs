...
An example of the type of question one is led to ask in this context is: how many matrix minors are sufficient to specify a point in $\Gr_+(k,n)$? In other words, given a $k \times n$ matrix $M$, how many minors of $M$ do we have to calculate to know if $M \in \Gr_+(k,n)$? These minors are not all independent; they satisfy identities known as Pl\"ucker relations,
\begin{equation}
...
\end{split}	 	
\end{equation} 
we can eliminate three of the nonadjacent minors---for instance, $\ket{24}$, $\ket{25}$, and $\ket{35}$---in terms of the remaining ${{5}\choose{2}} - 3 = 7$ adjacent and nonadjacent ones. It can be checked that all further Pl\"ucker relations are implied by those in~\eqref{eq:plucker}, telling us that all seven remaining minors must be computed to determine if a matrix is in $\Gr_+(2,5)$. However, as should be clear, it is not sufficient to check the positivity of {\it any} seven (ordered) minors; only certain triples of minors can be eliminated. It would therefore be advantageous to have a method for generating all sets of minors that are sufficient to answer this question.
...
Clusters can be defined to be quiver diagrams---namely, oriented graphs in which arrows connect different nodes---in which each node is assigned a cluster coordinate.\footnote{Here and throughout, we are implicitly restricting ourselves to skew-symmetric cluster algebras of geometric type~\cite{}.} We can form a cluster of $\Gr(2,5)$ out of our original triangulation~\eqref{eq:pent_triangulation} by assigning an orientation to the pentagon and all subtriangles, such as
...
In terms of these ingredients, a cluster algebra can be defined a set of clusters that is closed under mutation. This means that mutating on any non-frozen node of a cluster will generate a different cluster in the cluster algebra. One can thus generate a cluster algebra by starting with a seed such as~\eqref{eq:gr25-seed}, and iteratively mutating on all available nodes until the set of clusters closes (or you convince yourself that the cluster algebra is infinite). 

It is common to refer to certain cluster algebras by particularly nice representative clusters, when the mutable notes of the corresponding quiver form an oriented Dynkin diagram. For instance, the Gr(2,5) cluster algebra is often referred to as  $A_2$, since the mutable part of the seed~\eqref{eq:gr25-seed} is given by $\ket{13}\to\ket{14}$. Thus, we will often speak interchangeably of the cluster algebras for $\Gr(2,5)$ and $A_2$. This is a slight abuse of notation, as the $\Gr(2,5)$ cluster algebra corresponds specifically to the cluster algebra generated by the collection of frozen and mutable nodes in eq.~(\ref{eq:gr25-seed}), whereas an $A_2$ cluster algebra can in principle be dressed with any (non-zero) number of frozen nodes. We will see why this language is useful in the next section.
...
Clusters formed out of \xcoords\ can be directly constructed out of clusters involving \acoords. Given a quiver described by the exchange matrix $b_{ij}$, the $\a$- and $\x$-coordinates are related by
\begin{equation}
...
Just as with $\a$-coordinate clusters, we can generate all $\x$-coordinate clusters by mutation. Mutating on alternating nodes we generate the following sequence of clusters (where the coordinates colored \mbox{\color{red}{red}} denote which node is mutated on to get to the cluster in the following line):
\begin{align}
  x_1 &\to \color{red}{x_2} \nl
  ...
  &\vdots \nn
\end{align}
where the series then repeats, with all arrows reversed. Note that for $A_2$, if we label these $\x$-coordinates by
...
\end{equation}
the general mutation rule in~\eqref{eq:x-coord-mutation} takes the simple form
...
It's worth noting that cluster \xcoords\ come equipped with a natural Poisson bracket, making the Grassmannian $\Gr(k,n)$ a cluster Poisson variety~\cite{}. Namely, when two \xcoords\ appear together in a cluster of $\Gr(k,n)$, the Poisson bracket is given by
\begin{equation}
\{x_i, x_j \} = b_{ij} x_i x_j \ .
\end{equation}
This structure respects mutation, implying in particular that the entry $b_{ij}$ that counts the number of arrows between $x_i$ and $x_j$ in a given cluster's quiver will be the same in all clusters in which $x_i$ and $x_j$ both appear. The Poisson bracket (and associated Sklyanin bracket) will play a larger role in the forthcoming companion to this paper~\cite{}, so we postpone further discussion of this structure until then.

\subsection{Subalgebras}\label{sec:subalgebras}

Cluster algebras contain a rich and intricate subalgebra structure, which will play a central role in our analysis. We can motivate this structure by exploring the example of $\Gr(2,6)$, or equivalently the triangulations of a hexagon. In figure~\ref{eq:gr26-seed} we give the seed cluster for $\Gr(2,6)$ in the triangulation, \acoord, and \xcoord\ representations, respectively. Since the mutable nodes take the form of an $A_3$ Dynkin diagram, we often speak of $\Gr(2,6)$ and $A_3$ interchangeably, just as we did with $\Gr(2,5) \simeq A_2$. 
...
The $\Gr(2,6)$ cluster algebra features 14 clusters, which can be grouped together into multiple (overlapping) subalgebras. A simple example is the collection of all triangulations which involve the cord $\ket{15}$. It is easy to see that this set contains 5 clusters and is itself a cluster algebra generated by treating $\ket{15}$ in~\eqref{eq:gr26-seed} as a frozen node (in the \xcoord\ case, this corresponds to freezing the middle node). This of course is the cluster algebra corresponding to the triangulations of the pentagon formed by points $1,\ldots,5$:
\begin{equation}
\begin{gathered}
\begin{tikzpicture}
	\drawHexagon
...
\end{gathered}\ .
\end{equation}
We therefore refer to the collection of clusters that involve this pentagon as an $A_2$ subalgebra of $\Gr(2,6)$. Similarly, instead of freezing the chord $\ket{15}$ we could have frozen the chord $\ket{14}$, in which case we generate an $A_1 \times A_1$ subalgebra, since $A_1$ corresponds to the triangulations of a square and the chord $\ket{14}$ divides the hexagon in to two non-overlapping squares:
\begin{equation}
\begin{gathered}
\begin{tikzpicture}
	\drawHexagon
...
\end{gathered} \ .
\end{equation}
Larger cluster algebras contain many subalgebras of different types. In Appendix~\ref{appendix:subalgebras}, we catalog the number of subalgebras of different types that appear in the cluster algebras that are relevant to seven-particle scattering in planar ${\cal N} = 4$.
...
Similarly, the associahedron of $\Gr(2,6) \simeq A_3$ is given in figure~\ref{fig:a3-poly}. It has 14 vertices, corresponding to the 14 clusters of $\Gr(2,6)$, each with valency three, that assemble into three square faces and six pentagonal faces. This makes it easy to read off the subalgebra structure of $A_3$ directly---the square faces represent $A_1\times A_1$ subalgebras, while each pentagonal face represents an $A_2$ subalgebra. In this case, each of the faces corresponds to a distinct subalgebra. To make this more concrete, let's construct two of these subalgebras explicitly.

More generally, however, it is possible for subalgebras in different parts of the associahedron to share all the same mutable nodes.  

Associahedra for larger cluster algebras become quite complicated, and involve intricate subpolytope and subalgebra structures. For example, the associahedron for $\Gr(4,7)\simeq E_6$, which will be the focus of much of the rest of this paper, has 833 vertices, each of valence 6. Its two-dimensional faces are 1785 squares and 1071 pentagons, and there are 49 different $A$-coordinates that appear.
